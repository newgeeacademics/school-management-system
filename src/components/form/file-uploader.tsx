...
import type { FileUploaderProps } from '@/types/file-uploader';
...
export const FileUploader = ({
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!ALLOWED_TYPES.includes(file.type)) {
      setError('Veuillez envoyer des images PNG, JPG ou WEBP uniquement');
      return;
    }

    if (file.size > MAX_FILE_SIZE) {
      setError('La taille du fichier doit être inférieure à 3 Mo');
      return;
    }
          ...
          <div className='relative w-full rounded-xl border-2 border-blue-600/20 bg-gradient-to-r from-blue-50/50 to-blue-100/30 p-5'>
            <div className='flex flex-col sm:flex-row items-center gap-4'>
              ...
              <div className='flex-1 text-center sm:text-left'>
                <p className='text-sm font-bold text-gray-900'>Logo de l&apos;école</p>
                <p className='text-xs text-gray-900/60 mt-1'>Image claire et lisible.</p>
              </div>
          ...
          <section className='relative w-full rounded-xl overflow-hidden border-2 border-dashed border-gray-200 bg-blue-50/40 hover:border-blue-400 hover:bg-blue-50/30 transition-all duration-300 cursor-pointer'>
            ...
            <label htmlFor={`file-upload-${type}`} className='cursor-pointer block'>
              <div className='flex flex-col sm:flex-row items-center gap-4 p-5'>
                ...
                <div className='flex-1 text-center sm:text-left'>
                  <p className='text-sm font-bold text-blue-600 mb-1'>Cliquez pour ajouter le logo</p>
    ...
    <>
      {previewUrl ? (
        <section className='relative w-full'>
          ...
          />
          <Button type='button' variant='destructive' size='sm' onClick={handleRemove} className='absolute top-2 right-2'>
            Retirer
          </Button>
        </section>
      ) : (
        <section className='flex aspect-[4/1] flex-col items-center justify-center p-8 rounded-xl border-2 border-dashed border-gray-200 bg-gray-50 cursor-pointer hover:border-blue-400 hover:bg-blue-50/50 transition-all duration-300'>
          <input
            ...
            className='hidden'
          />
          <label htmlFor={`file-upload-${type}`} className='cursor-pointer text-center w-full'>
            <Upload className='h-12 w-12 mx-auto mb-3 text-blue-600' />
            <p className='text-sm font-bold text-gray-900 mb-1'>
              <span className='text-blue-600'>Cliquez pour envoyer</span>
            </p>
            <p className='text-xs text-gray-900/60'>{maxSizeText}</p>
          </label>
        </section>
