...
export function StudentCreateWizard({
  ...
  const canContinue = React.useCallback(
    (currentStep: number) => {
      switch (currentStep) {
        case 1:
          return Boolean(form.firstName.trim() && form.lastName.trim()) && !enrolledCheck.isOver;
        case 2:
          return !enrolledCheck.isOver;
        case 3:
          return (
            hasContact &&
          ...
          {enrolledCheck.licensed != null ? (
            ...
            >
              <p>
                ...
                <span className='font-medium text-foreground'>{enrolledCheck.licensed}</span>
              </p>
              {enrolledCheck.isOver ? (
                <p className='mt-1'>
                  {enrolledOverMessage(enrolledCheck.licensed, enrolledCount)}{' '}
                  {onGoToBilling ? (
                    <button
                      type='button'
